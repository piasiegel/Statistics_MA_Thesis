...
#---- fit model ----
m_RT2 <- lmer(log(response_time) ~ condition * congruency + run + Lesion_volume_z + (1+ run + condition * congruency|sub) + (1|stimulus_audio) + (1|stimulus_picture), data = df_RT, REML = F, lmerControl(optimizer = "bobyqa"))
summary(m_RT1)

drop1(m_RT1, test = "Chisq")

plot(ggemmeans(m_RT1, terms = c("condition", "congruency")))
plot(ggemmeans(m_RT1, terms = c("Lesion_volume_z")), limit.range = T, collapse.group = T)
```
...
plot(ggeffect(m_acc1, terms = c("condition", "congruency")))
plot(ggeffect(m_acc1, terms = c("Lesion_volume_z")), collapse.group = T, limit.range = T)
```
