...
df_npsy <- read.table(here::here("raw_data/Npsy_data.txt"), header = T, sep = "\t")
...
# sort columns
all_files <- all_files[,c(1,21:29,2:20,30:46)]

# save df
save(all_files, file = 'RData/all_files_11_2023.RData')
```
...
m_RT_res <- drop1(m_RT2, test = "Chisq")
save(m_RT_res, file = here::here(paste0("RData/m_RT_res", today, ".RData")))

RT_int <- plot(ggemmeans(m_RT1, terms = c("condition", "congruency")), collapse.group = "sub") +
...
RT_int
save_plot(RT_int, file = here::here(paste0("Plots/m_RT_cond_cong_", today, ".pdf")))

RT_lesionVol <- plot(ggemmeans(m_RT1, terms = c("Lesion_volume_z")), limit.range = T, collapse.group = "sub") +
  ylab("Reaction time in ms") +
...
RT_run <- plot(ggemmeans(m_RT1, terms = c("run")), collapse.group = "sub") +
  ylab("Reaction time in ms") +
  xlab("Run") +
  apatheme +
   theme(legend.position = "none",
...
#---- Model ----
m_acc1 <- glmer(correct ~ condition * congruency + run + Lesion_volume_z + (1 + run + condition + congruency|sub) + (1|stimulus_audio), data = df_behav, family = binomial(link = "logit"), glmerControl(optimizer = "bobyqa")) # model with interaction in random slopes is singular
...
Acc_int <- plot(ggemmeans(m_acc1, terms = c("condition", "congruency")), collapse.group = "sub") +
  scale_colour_manual(values = palet_task) +
...
Acc_lesionVol <- plot(ggemmeans(m_acc1, terms = c("Lesion_volume_z[all]")), limit.range = T) +
  ...
  theme(legend.position = "none")
Acc_lesionVol
save_plot(Acc_lesionVol, file = here::here(paste0("Plots/m_Acc_lesionVol_", today, ".pdf")))
```
