...
>>>>>>> fd0c28dc50e0cc02297774666dc501e5d63ec58d
                       'Age',
                       'WSO',
                       'Lesion_volume',
                       'accuracy_num')]
...
# Create .csv-dataframe for rt (no distinction between conditions; SVR-LSM)
```{r}
## Create new data frame with relevant variables 
data_rt <- data_raw[c('RegistryCode',
                       'Age',
                       'WSO',
                       'Lesion_volume',
                       'response_time')]
...
df_rt_summary <- data_rt %>% 
  group_by(RegistryCode) %>% 
  mutate(average_rt = mean(response_time)) %>% 
  select(-response_time) %>% 
  distinct()

## Extract data frame "data_rt.csv"
write.csv(df_rt_summary, here("./RData/data_rt.csv"), row.names = F)
```
