...
```{r}
# remove NA values
df_RT <- df_new[!is.na(df_new$response_time),]
...
#---- fit model ----
m_RT <- lmer(log(response_time) ~ condition * congruency + run + lesion_volume_z + age_z + (1+ run + condition * congruency|sub) + (1|stimulus_audio) + (1|stimulus_picture), data = df_RT, REML = F, lmerControl(optimizer = "bobyqa"))
